...
//					String id =  modelJsonObject.getString(varField);
//					if(StringUtil.isBlank(id))
//						continue;
//					Integer varValue =Integer.parseInt(id);
...
//				}else {
				...
				if(varField.equals("attach")){
					JSONArray array = modelJsonObject.getJSONArray(varField);
					varField =  jsonArray2str(array.toString());
				}else {
					varValue = modelJsonObject.get(varField);
				}
				field.setAccessible(true);
				field.set(modelObj, varValue);
//				}
	...
	public static User json2user(String result) {
		// TODO Auto-generated method stub
		try {
			JSONObject jsonObject = new JSONObject(result);
			return  (User) json2model(C.model.packet+"User", jsonObject);
		} catch (JSONException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
	...
	public static String jsonArray2str(String jsonArray){
		if(StringUtil.isBlank(jsonArray) && jsonArray.length()>2)
			return "";
		else
